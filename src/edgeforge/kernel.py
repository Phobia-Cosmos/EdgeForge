...
class KernelSpec:
    ...
    def from_payload(cls, payload: dict[str, Any]) -> "KernelSpec":
        ...
        if not kernel_id:
            identity = json.dumps(
                {"operator": operator, "backend": backend, "version": version, "architectures": architectures, "dtypes": dtypes},
                sort_keys=True,
                separators=(",", ":"),
            ).encode("utf-8")
        ...
        return cls(kernel_id, operator, backend, version, architectures, dtypes, constraints, metadata)
...
def kernel_supports_worker(kernel: dict[str, Any], worker: dict[str, Any]) -> bool:
    ...
    return "*" in supported or architecture in supported
