...
This module is an integration baseline rather than an optimized compiler. It
is packaged with EdgeForge so a Worker can run it from an isolated work root.
"""
...
def _infer(model: dict[str, Any], samples: list[list[float]]) -> list[list[float]]:
    ...
    for sample in samples:
        logits = [
            sum(left * right for left, right in zip(sample, row)) + bias
...
def export(root: Path) -> dict[str, Any]:
    model = {
        ...
        "classes": 4,
        "weights": [
            [math.sin((row + 1) * (column + 1)) * 0.1 for column in range(16)]
            for row in range(4)
        ],
    ...
    return {
        ...
        "path": str(root / "exported-model.json"),
    }


def transform(root: Path) -> dict[str, Any]:
    raw = [
        [math.sin(sample * 0.17 + feature * 0.11) + 0.01 * sample for feature in range(16)]
    ...
    dataset = {
        "format": "edgeforge-synthetic-eeg-v1",
        "samples": normalized,
        "window_length": 16,
        "stride": 16,
    }
    ...
    return {
        ...
        "features": 16,
        "path": str(root / "transformed-dataset.json"),
    }
...
def compile_reference(root: Path) -> dict[str, Any]:
    ...
    compiled = {
        ...
        "compiler_identity": "edgeforge-reference-v1",
        "model": model,
    ...
    return {
        ...
        "path": str(root / "compiled-model.json"),
    }
...
def run(root: Path) -> dict[str, Any]:
    ...
    return {
        "outputs": len(outputs),
        "output_digest": _digest(document),
        "path": str(root / "outputs.json"),
    }
...
def benchmark(root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "summary": {
            ...
            "p95_ms": round(sorted(timings)[18], 6),
        },
