...
EdgeForge 使用语义化版本 `MAJOR.MINOR.PATCH`。协议不兼容、任务或 IR 语义不兼容时增加 MAJOR；增加向后兼容能力时增加 MINOR；仅修复兼容问题时增加 PATCH。当前开发线为 `0.16.3` ISRUC EEG LoP dose/retention validation snapshot，V2–V16 的契约保持兼容；Orange Pi/P550 的真实 EEG 模型 Runtime 证据仍需单独完成后才能标记部署稳定。
