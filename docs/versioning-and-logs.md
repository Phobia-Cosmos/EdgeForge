...
EdgeForge 使用语义化版本 `MAJOR.MINOR.PATCH`。协议不兼容、任务或 IR 语义不兼容时增加 MAJOR；增加向后兼容能力时增加 MINOR；仅修复兼容问题时增加 PATCH。当前开发线为 `0.11.0` 候选版，V2–V10 已通过自动化验证并冻结各自契约；V11 必须在 Orange Pi 真实证据完成后才能标记稳定。
