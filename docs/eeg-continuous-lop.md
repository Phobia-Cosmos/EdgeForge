...
The output contains raw/per-epoch-standardized waveforms, channel-averaged mean-per-epoch Welch spectra, an epoch-level RMS heatmap with the adaptation/evaluation boundary, PCA of log-RMS and band-power features, a gain-invariant PCA after per-epoch RMS normalization, and an adaptation/evaluation label-prior heatmap. `visualization-summary.json` additionally records the target RMS max/min ratio, per-subject delta/theta/alpha/sigma/beta fractions, Jensen–Shannon spectral distance to the target median, and both PCA summaries. These are drift diagnostics, not LoP outcomes; a visual separation can be caused by amplitude scale, frequency content, or label prior and must be paired with equal-budget fresh/warm probes. See [EEG/image feature comparison](eeg-image-feature-comparison.md) for interpretation guidance.
