...
class BackendRegistryTests(unittest.TestCase):
    def test_registry_lists_explicit_target_contracts(self):
        ...
        self.assertTrue({"python-reference", "onnx-runtime", "iree", "rknn", "opencl", "vulkan"}.issubset(names))
    ...
    def test_vulkan_and_opencl_require_explicit_supported_architecture(self):
        with self.assertRaisesRegex(ValueError, "architecture"):
            validate_backend_target("vulkan", {})
        self.assertEqual(validate_backend_target("vulkan", {"architecture": "aarch64"})["name"], "vulkan")
        self.assertEqual(validate_backend_target("opencl", {"architecture": "x86_64"})["name"], "opencl")
